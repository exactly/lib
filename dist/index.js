...
__export(src_exports, {
  INTERVAL: () => INTERVAL,
  MAX_UINT256: () => MAX_UINT256_default,
  WAD: () => WAD_default,
  abs: () => abs,
  ...
  fixedUtilization: () => fixedUtilization,
  floatingDepositRates: () => floatingDepositRates,
  floatingRate: () => floatingRate,
  floatingUtilization: () => floatingUtilization,
...
function borrowLimit(data, market, timestamp = Math.floor(Date.now() / 1e3), targetHealthFactor = BigInt(1.25 * 10 ** 18)) {
...
function healthFactor(data, timestamp = Math.floor(Date.now() / 1e3)) {
  const { adjCollateral, adjDebt } = accountLiquidity(data, timestamp);
  return adjDebt ? divWad(adjCollateral, adjDebt) : MAX_UINT256_default;
}

// src/auditor/withdrawLimit.ts
function withdrawLimit(data, market, timestamp = Math.floor(Date.now() / 1e3), targetHealthFactor = BigInt(1.25 * 10 ** 18)) {
  const { adjCollateral, adjDebt } = accountLiquidity(data, timestamp);
  const marketData = data.find(({ market: m }) => m.toLowerCase() === market.toLowerCase());
  if (!marketData) throw new Error("market not found");
...
function fixedRate(maturity, maxPools, uFixed, uFloating, uGlobal, parameters, timestamp = Math.floor(Date.now() / 1e3), base = baseRate(uFloating, uGlobal, parameters), z) {
  ...
  const maturityFactor = BigInt(maturity - timestamp) * WAD_default / BigInt(maxPools * INTERVAL);
...
function splitInstallments(totalAmount, totalAssets, firstMaturity, maxPools, uFixed, uFloating, uGlobal, parameters, timestamp = Math.floor(Date.now() / 1e3), { power = WAD_default * 60n / 100n, scaleFactor = WAD_default * 95n / 100n, tolerance = 20n, maxIterations = 66666n } = {}) {
  ...
  const maturityFactors = rates.map(
    (_, index) => BigInt(firstMaturity + index * INTERVAL - timestamp) * WAD_default / ONE_YEAR
  );
  const y = mul(installments, WAD_default);
  let effectiveRate = rates[0];
  iterations = 0;
  error = 0n;
  do {
    if (iterations++ >= maxIterations) throw new Error("MAX_ITERATIONS_EXCEEDED");
    const aux = add(mulDivUp2(maturityFactors, effectiveRate, WAD_default), WAD_default);
...
function fixedRates(firstMaturity, maxPools, uFixed, uFloating, uGlobal, parameters, timestamp = Math.floor(Date.now() / 1e3)) {
