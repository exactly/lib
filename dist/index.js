...
__export(src_exports, {
  MATURITY_INTERVAL: () => MATURITY_INTERVAL,
...
function borrowLimit(data, market, targetHealthFactor = WAD_default * 105n / 100n, timestamp = Math.floor(Date.now() / 1e3)) {
...
function withdrawLimit(data, market, targetHealthFactor = WAD_default * 105n / 100n, timestamp = Math.floor(Date.now() / 1e3)) {
  const { adjCollateral, adjDebt } = accountLiquidity(data, timestamp);
  const marketData = data.find(({ market: m }) => m.toLowerCase() === market.toLowerCase());
  if (!marketData) throw new Error("market not found");
