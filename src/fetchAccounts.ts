...
const mapAccountsResponse = (response: AccountResponse[], address: string, markets: Market[]) => (
  response.map(({
    borrowShares, depositShares, fixedPositions, market: marketId,
  }) => {
    ...
    return ({
      ...
      fixedPositions: fixedPositions.map(({
        principal, fee, borrow, maturity, rate,
      }) => ({
        borrow,
        maturity,
...
export default async (subgraph: string, address: string) => {
  const { accounts: accountsResponse, markets } = await request<{ accounts: AccountResponse[], markets: Market[] }>(subgraph, `{
